...
######################### CONSTANTS #########################
# Constants
SUBMIT_PREDICTION = True  # set to True to publish your predictions to 
Metaculus
USE_EXAMPLE_QUESTIONS = True  # set to True to forecast example questions 
rather than the tournament questions
NUM_RUNS_PER_QUESTION = 5  # The median forecast is taken between 
NUM_RUNS_PER_QUESTION runs
SKIP_PREVIOUSLY_FORECASTED_QUESTIONS = False

# Environment variables
# You only need *either* Exa or Perplexity or AskNews keys for online 
research
...
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY") # You'll also need the OpenAI 
API Key if you want to use the Exa Smart Searcher
...
# The example questions can be used for testing your bot. (note that 
question and post id are not always the same)
EXAMPLE_QUESTIONS = [  # (question_id, post_id)
    (8466, 8466)
]

# Also, we realize the below code could probably be cleaned up a bit in a 
few places
# Though we are assuming most people will dissect it enough to make this 
not matter much
...
def post_question_prediction(question_id: int, forecast_payload: dict) -> 
None:
    """
...
def create_forecast_payload(
    ...
    """
    Accepts a forecast and generates the api payload in the correct 
format.
...
"anthropic/claude-3.5-sonnet", temperature: float = 0.3) -> str:
    """
    Makes a completion request to OpenRouter's API with concurrent request 
limiting.
    """
    client = AsyncOpenAI(
        base_url="https://openrouter.ai/api/v1",
        api_key=OPENROUTER_API_KEY,  # Assumes OPENROUTER_API_KEY is set 
in environment
        max_retries=2,
    )
...
def call_perplexity(question: str) -> str:
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",  # this is a system prompt designed to 
guide the perplexity assistant
                "content": """
                You are an assistant to a superforecaster.
                The superforecaster will give you a question they intend 
to forecast on.
                To be a great assistant, you generate a concise but 
detailed rundown of the most relevant news, including if the question 
would resolve Yes or No based on current information.
                You do not produce forecasts yourself.
                """,
            },
            {
                "role": "user",  # this is the actual prompt we ask the 
perplexity assistant to answer
                "content": question,
            },
        ],
    }
...
def call_exa_smart_searcher(question: str) -> str:
    if OPENAI_API_KEY is None:
        ...
        highlights = 
asyncio.run(searcher.invoke_for_highlights_in_relevance_order(question))
...
"{highlight.highlight_text}"\n\n'
        ...
        prompt = (
            "You are an assistant to a superforecaster. The 
superforecaster will give"
            "you a question they intend to forecast on. To be a great 
assistant, you generate"
            "a concise but detailed rundown of the most relevant news, 
including if the question"
            "would resolve Yes or No based on current information. You do 
not produce forecasts yourself."
...
https://docs.asknews.app/en/reference#get-/v1/news/search
    """
    ask = AskNewsSDK(
        client_id=ASKNEWS_CLIENT_ID, client_secret=ASKNEWS_SECRET, 
scopes=set(["news"])
    )
    ...
    hot_response = ask.news.search_news(
        ...
        n_articles=6,  # control the number of articles to include in the 
context, originally 5
        return_type="both",
        strategy="latest news",  # enforces looking at the latest news 
only
    )

    # get context from the "historical" database that contains a news 
archive going back to 2023
    historical_response = ask.news.search_news(
        query=question,
        n_articles=10,
        return_type="both",
        strategy="news knowledge",  # looks for relevant news within the 
past 60 days
    )
    ...
    if hot_articles:
        ...
        hot_articles = sorted(hot_articles, key=lambda x: x["pub_date"], 
reverse=True)
...
{pub_date}\nSource:[{article['source_id']}]({article['article_url']})\n\n"
    ...
    if historical_articles:
        historical_articles = [article.__dict__ for article in 
historical_articles]
...
weight on the status quo outcome since the world changes slowly most of 
the time.

The last thing you write is your final answer as: "Probability: ZZ%", 
0-100
"""
...
def extract_probability_from_response_as_percentage_not_decimal(
    ...
    if matches:
        ...
        number = min(99, max(1, number))  # clamp the number between 1 and 
99
        return number
    else:
        raise ValueError(f"Could not extract prediction from response: 
{forecast_text}")
...
async def get_binary_gpt_prediction(
    ...
    async def get_rationale_and_probability(content: str) -> tuple[float, 
str]:
...
- Please notice the units requested (e.g. whether you represent a number 
as 1,000,000 or 1m).
- Never use scientific notation.
- Always start with a smaller number (more negative if negative) and then 
increase from there
...
(e) A brief description of an unexpected scenario that results in a low 
outcome.
(f) A brief description of an unexpected scenario that results in a high 
outcome.

You remind yourself that good forecasters are humble and set wide 90/10 
confidence intervals to account for unknown unkowns.
...
def extract_percentiles_from_response(forecast_text: str) -> dict:
    ...
    # Helper function that returns a list of tuples with numbers for all 
lines with Percentile
        ...
        for line in text.split("\n"):
            if re.match(pattern, line):
                ...
                if len(numbers) > 1:
                    ...
                    # Check if the original line had a negative sign 
...
def generate_continuous_cdf(
    ...
    if open_upper_bound:
        if range_max > percentile_values[percentile_max]:
            percentile_values[int(100 - (0.5 * (100 - percentile_max)))] = 
range_max
    else:
    ...
    def generate_cdf_locations(range_min, range_max, zero_point):
        if zero_point is None:
            ...
            deriv_ratio = (range_max - zero_point) / (range_min - 
zero_point)
    ...
    def linear_interpolation(x_values, xy_pairs):
        ...
        for x in x_values:
            ...
            if x in known_x:
                ...
                # If x is outside the range of known x-values, use the 
nearest endpoint
                if i == 0:
...
async def get_numeric_gpt_prediction(
    ...
    unit_of_measure = question_details["unit"] if question_details["unit"] 
    ...
    if open_upper_bound:
        ...
        upper_bound_message = f"The outcome can not be higher than 
{upper_bound}."
    if open_lower_bound:
        lower_bound_message = ""
    else:
        lower_bound_message = f"The outcome can not be lower than 
{lower_bound}."
...
weight on the status quo outcome since the world changes slowly most of 
the time, and (2) good forecasters leave some moderate probability on most 
options to account for unexpected outcomes.

The last thing you write is your final probabilities for the N options in 
...
def extract_option_probabilities_from_response(forecast_text: str, 
options) -> float:
...
lines with Percentile
    def extract_option_probabilities(text):
        ...
        for line in text.split("\n"):
            ...
            numbers = [
                float(num) if "." in num else int(num) for num in 
numbers_no_commas
            ]
...
def generate_multiple_choice_forecast(options, option_probabilities) -> 
dict:
    """
    ...
    if len(options) != len(option_probabilities):
        raise ValueError(
            f"Number of options ({len(options)}) does not match number of 
probabilities ({len(option_probabilities)})"
        )
    ...
    for i in range(len(options)):
        probability_yes_per_category[options[i]] = 
normalized_option_probabilities[i]
...
Answer: "
    ...
    probability_yes_per_category_and_comment_pairs = await asyncio.gather(
        *[ask_llm_for_multiple_choice_probabilities(content) for _ in 
range(num_runs)]
    )
    comments = [pair[1] for pair in 
probability_yes_per_category_and_comment_pairs]
    final_comment_sections = [
        f"## Rationale {i+1}\n{comment}" for i, comment in 
enumerate(comments)
    ]
    ...
    final_comment = (
        f"Average Probability Yes Per Category: 
...
in the question data.
    ...
    question.my_forecasts.latest.forecast_values has the following values 
...
...]
    ...
    try:
        forecast_values = 
post_details["question"]["my_forecasts"]["latest"][
            "forecast_values"
        ]
...
async def forecast_individual_question(
    ...
    summary_of_forecast += 
f"-----------------------------------------------\nQuestion: {title}\n"
    summary_of_forecast += f"URL: 
https://www.metaculus.com/questions/{post_id}/\n"
...
{post_id} Question {question_id}:\n")
    print(f"Forecast for post {post_id} (question 
{question_id}):\n{forecast}")
    print(f"Comment for post {post_id} (question 
{question_id}):\n{comment}")
    ...
    if submit_prediction == True:
        forecast_payload = create_forecast_payload(forecast, 
question_type)
        post_question_prediction(question_id, forecast_payload)
        post_question_comment(post_id, comment)
        summary_of_forecast += "Posted: Forecast was posted to 
Metaculus.\n"
...
async def forecast_questions(
    ...
    forecast_summaries = await asyncio.gather(*forecast_tasks, 
    ...
    for question_id_post_id, forecast_summary in zip(
        ...
        if isinstance(forecast_summary, Exception):
            print(
                f"-----------------------------------------------\nPost 
{post_id} Question {question_id}:\nError: 
{forecast_summary.__class__.__name__} {forecast_summary}\nURL: 
https://www.metaculus.com/questions/{post_id}/\n"
            )
